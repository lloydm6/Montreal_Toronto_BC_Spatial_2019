...
mark.lm.to.u10k.11vars.seqrep <- lm(data = dplyr::filter(mark.t.s.data.stan, bc_conc < 10000), bc_conc ~ open_50m + Nox_50m + I(resid_100m^2) +
                                     build_200m:ind_1000m + com_750m:resid_100m + com_750m:ind_1000m + resid_100m:ind_1000m + ind_1000m:Nox_50m +
                                     ind_1000m:d_airport + d_highway:Nox_50m + Nox_50m:rail_1000m)
summary(mark.lm.to.u10k.11vars.seqrep)
```


```{r results = 'asis'}

stargazer(mark.lm.to.u10k.9vars.ex, mark.lm.to.u10k.9vars.seqrep, mark.lm.to.u10k.11vars.seqrep, ci = FALSE, ci.level = 0.95, type = "html")
